...
CURRENT STATE: nothing scaffolded yet. A fresh Kotlin/Compose/Media3/Room project
should be created once Android Studio + JDK + the Android SDK are actually installed
and confirmed working on the user's machine - not guessed at blind, the same
discipline that governed the Expo pass (verify with a real toolchain before writing
source that depends on it, per the "research before guessing" pattern that caught
real bugs during that attempt - see below).
...
1. Install/confirm Android Studio + JDK + Android SDK on the user's machine, then
   scaffold the new Kotlin/Compose/Media3/Room project at the repo root.
2. Chromaprint fingerprint *generation* - see the dedicated section below; needs a
   real native module (JNI binding to libchromaprint) once a device/build loop
