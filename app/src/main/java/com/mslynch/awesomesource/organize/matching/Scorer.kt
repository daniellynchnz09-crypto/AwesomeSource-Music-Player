 ...
 * be treated as no match. Pure logic, no I/O - see [ScorerTest] (ported from
 * `tests/test_scorer.py`) for the same edge cases the Python version already found
 * and fixed once.
 *
 * See [FuzzyMatch]'s doc comment: backed by a real, published fuzzywuzzy port
 * (`me.xdrop:fuzzywuzzy`), not a from-scratch reimplementation - the thresholds
 * below are the Python original's unmodified values, verified to need no
 * recalibration against a real fuzzy-matching library during the Expo/TypeScript
 * attempt (`fuzzball`, the JS equivalent of this same library).
 */
object Scorer {
