 ...
 * same reasoning in each comment. See [FuzzyMatch]'s doc comment: backed by a real
 * published fuzzywuzzy port, not a from-scratch reimplementation, so these
 * thresholds are the Python original's unmodified values (verified to need no
 * recalibration during the Expo/TypeScript attempt's equivalent port).
 */
class ScorerTest {
